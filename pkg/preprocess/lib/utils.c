...
!.desc
! header_anyfile(frame,name,mode)	
!
! Anyfile *frame;       image structure
! char *name;           frame name
! char *mode;           open mode (Input,Ouput,IO)
!.ed
-------------------------------------------------------------------- */
int 
open_headerfile(Anyfile *frame, char *name, char *mode)		
{
  char errtext[132], filename[lg_name+1];
  ...
  memset(anyfile->ident,' ',lg_ident);
  anyfile->ident[lg_ident] = '\0';
  memset(anyfile->cunit,' ',lg_unit);
  anyfile->cunit[lg_unit] = '\0';
  memset(anyfile->history,' ',lg_hist);
  anyfile->history[lg_hist] = '\0';
  anyfile->external_info = NULL;
  anyfile->file_type = IMA_TYPE;
  anyfile->data_format = InputIO.basic_io;
  anyfile->data.d_data = NULL;
  anyfile->min = 0;
  anyfile->max = 0;
  anyfile->nwcs = 0;
  anyfile->wcs = NULL;
  ...
  strcpy(filename,name);
  first_blk(filename); 
  strcpy(anyfile->name,filename);
  append_ima_extension(anyfile->name,InputIO.basic_io);

  strcpy(filename,anyfile->name);

  if (!exist(filename)) {
    status = ERR_OPEN;
    sprintf(errtext,"header_anyfile: anyfile %s",filename);
    Handle_Error(errtext,status);
    return(status);
  }
  ...
  case 'I' : 
    if (mode[1] == 'O')
      anyfile->iomode = (int)IO_MODE;
    else
      anyfile->iomode = (int)I_MODE;
    break;
  case 'O' : anyfile->iomode = (int)O_MODE;
    break;
  default  : anyfile->iomode = (int)I_MODE;
    break;
  }
	
  iomode = get_iomode_code(InputIO.basic_io,anyfile->iomode);
  ...
  case STSDAS_FORMAT :
    len = strlen(filename);
    uimopn(filename,&iomode,&(anyfile->imno),&status,len);
    if (status != 0) 
      break;
    /*    uimgid(&(anyfile->imno),&int_datatype,&two_dim,&(anyfile->nx),&status);
    anyfile->data_type = decode_datatype(InputIO.basic_io,(short)(int_datatype)); */
    break;
#endif
#ifdef FITS
  case FITS_A_FORMAT :
  case FITS_B_FORMAT :
    ...
    anyfile->external_info = (void *)fptr;
  ...
  if (status) {
    sprintf(errtext,"header_anyfile: anyfile %s",filename);
    status = get_tiger_errcode(anyfile->data_format,status);
    Handle_Error(errtext,status);
  }
  else {
    disable_user_warnings();
    RD_desc(anyfile,"HISTORY",CHAR,lg_hist,anyfile->history);
    restore_user_warnings();

    status = 0;
    /* parse wcs if contained in file */
    status = parse_wcs(anyfile);
  }
  return(status);
}
...
!.func                       close_anyfile()
!
!.purp             closes a currently active anyfile (header only file) 
!.desc
! close_anyfile(anyfile)	
!
! IMAGE2D *anyfile;       image structure
!.ed
-------------------------------------------------------------------- */
int 
close_headerfile(Anyfile *anyfile)			/* close active anyfile */
{
  char   errtext[132], filename[lg_name+1];
  int    stat, int_datatype;
  float  cuts[4];
  ...
  if (anyfile->iomode == (int)I_MODE) {
    switch (anyfile->data_format) {
#ifdef MIDAS
    case MIDAS_FORMAT :
      stat = SCFCLO(anyfile->imno);
      break;
#endif
#ifdef IRAF
    case IRAF_FORMAT :
    case STSDAS_FORMAT :
      uimclo(&(anyfile->imno),&stat);
      break;
#endif
#ifdef FITS
    case FITS_A_FORMAT :
    case FITS_B_FORMAT :
      stat =0;
      fptr = (fitsfile *)anyfile->external_info;
      fits_close_file(fptr,&stat);
      anyfile->external_info = NULL;
      break;
#endif
    ...
    if (stat) {
      sprintf(errtext,"close_anyfile: anyfile %s",filename);
      stat = get_tiger_errcode(anyfile->data_format,stat);
      Handle_Error(errtext,stat);
    }
    return(stat);
  }

  WR_history(anyfile, (Anyfile *)0);
  ...
  if (stat) {
    ...
    if (TK && (anyfile->iomode == O_MODE || anyfile->iomode == IO_MODE))
      {
        printf("@ N {%s}\n",filename);
      }
  }
